...
import { startTelemetry, shutdownTelemetry, isTelemetryActive } from '../src/observability.js';
...
describe('Observability', () => {
  ...
  describe('Error Tracking Integration', () => {
    it('should initialize without Sentry DSN configured', async () => {
      ...
      const { initializeSentry } = await import('../src/error-tracking.js');
      expect(() => initializeSentry()).not.toThrow();
    });

    it('should provide error capture functions', async () => {
      const { captureException, captureMessage } = await import('../src/error-tracking.js');
