...
type ImportResult = { title: string; text: string };
type Lesson = { title: string };
type Draft = {
  lessons: Lesson[];
  landing: { hero: { sub: string } };
  [k: string]: any;
};
...
export default function MarketingScreen() {
  ...
  } = useForm<z.infer<typeof GenerateSchema>>({
  ...
  return (
    <main className="mx-auto max-w-3xl p-8">
      ...
      <section className="mb-10">
        ...
        <form
          onSubmit={handleSubmitImport(onImport)}
        ...
        {importResult && (
          <div className="mt-4 rounded-lg bg-neutral-100 p-4">
            ...
            <p className="mt-2 text-sm text-neutral-700 line-clamp-5">
              {importResult.text}
            </p>
          </div>
        )}
      ...
      <section className="mb-10">
        ...
        <form
          onSubmit={handleSubmitGenerate(onGenerate)}
          className="flex flex-col gap-4"
        >
          <input
            className="rounded-lg border p-3"
          ...
          />
          {errorsGenerate.title && (
            <p className="text-red-500">Course title required</p>
          )}
          <textarea
            className="rounded-lg border p-3"
            rows={6}
          ...
          />
          {errorsGenerate.body && (
            <p className="text-red-500">Content too short</p>
          )}
          <button
            type="submit"
            className="rounded-lg bg-black px-4 py-2 text-white hover:bg-neutral-800"
        ...
        {draft && (
          <div className="mt-6 space-y-6">
            ...
            <p className="italic text-neutral-600">
              Landing tagline: {draft.landing.hero.sub}
            </p>
          </div>
        )}
      </section>
      ...
      {draft && (
        <section className="border-t pt-6">
          ...
          <p className="mb-2 text-neutral-700">
            ...
            wire a slug and store the course in the database (e.g., using a
            server action or API route). Then create a Stripe price and call the
            checkout endpoint. A template for that flow is in the README.
          </p>
          <button
            disabled
