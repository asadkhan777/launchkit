...
Before any code generation, ask: _"Does this change advance the North Star Goal?"_  
...
AI agents should exclude these directories and file patterns from analysis, reference and lookup to reduce noise (unless explicitly instructed otherwise):
...
- **Import Resilience**: Use path aliases (`@/module`) over relative imports to prevent Shotgun Surgery anti-pattern when refactoring
